 ...
 * ADD | cout ( result > 0xFF )
 * SUB | cout ( result > 0xFF )
 * XOR | 
 * AST | cout
 */
...
// Miscellenious
void MSC(QCPU *CPU, uint8_t inst) { CPU->running = (inst != 0); }
// Storage Store Data
void SST(QCPU *CPU, uint8_t inst) { CPU->storageStore(CPU->dCache, (inst<<5)|(CPU->poi&0xF8)); }
// Storage Load Data
void SLD(QCPU *CPU, uint8_t inst) { CPU->storageLoad(CPU->dCache, (inst<<5)|(CPU->poi&0xF8)); }
...
void LIM(QCPU *CPU, uint8_t inst) {
    if(inst == 0) CPU->acc = CPU->iCache[CPU->pc++];
    else CPU->regs[inst] = CPU->iCache[CPU->pc++];
}
// Register Store Accumulator
void RST(QCPU *CPU, uint8_t inst) { CPU->regs[inst] = CPU->acc; }
// Accumulator Store Register
void AST(QCPU *CPU, uint8_t inst) {
    uint16_t result;
    if(inst == 6) result = CPU->regs[inst]++;
    else result = CPU->regs[inst];
...
void POI(QCPU *CPU, uint8_t inst) { CPU->poi = inst; }
// No Operation
void NOP(QCPU *CPU, uint8_t inst) {  }
// Jump
void JMP(QCPU *CPU, uint8_t inst) { if(!((~CPU->_Flags) & CPU->selFlags)) { CPU->pc = inst; } }  
// Store Acc To Memory
void MST(QCPU *CPU, uint8_t inst) { CPU->dCache[inst] = CPU->acc; }
// Load To Acc From Memory
void MLD(QCPU *CPU, uint8_t inst) { CPU->acc = CPU->dCache[inst]; }
